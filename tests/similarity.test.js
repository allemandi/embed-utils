import { computeCosineSimilarity, computeEuclideanDistance, computeManhattanDistance, normalizeVector, isNormalized, meanVector } from '../src/utils/similarity';
...
describe('computeManhattanDistance', () => {
    test('returns 0 for identical vectors', () => {
        const a = [1, 2, 3];
        const b = [1, 2, 3];
        expect(computeManhattanDistance(a, b)).toBe(0);
    });
    test('returns correct distance for simple positive vectors', () => {
        const a = [1, 2];
        const b = [2, 3];
        expect(computeManhattanDistance(a, b)).toBe(2);
    });
    test('returns correct distance for vectors with negative values', () => {
        const a = [1, -1];
        const b = [-1, 1];
        expect(computeManhattanDistance(a, b)).toBe(4);
    });
    test('returns correct distance when one vector has zeros', () => {
        const a = [0, 0];
        const b = [1, 2];
        expect(computeManhattanDistance(a, b)).toBe(3);
    });
});
