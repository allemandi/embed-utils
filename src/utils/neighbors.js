...
/**
 * Available distance/similarity methods for neighbor search
 * @typedef {'cosine' | 'euclidean' | 'manhattan'} DistanceMethod
 */
 ...
 * @param {DistanceMethod} [options.method='cosine'] - Distance/similarity method to use. Default is 'cosine'.
...
function findNearestNeighbors(queryEmbedding, samples, options = {}) {
    ...
    // Compute scores/distances and filter by threshold
    for (const sample of samples) {
        const score = computeFn(queryEmbedding, sample.embedding);
        
        // Apply threshold filter if threshold is provided
        if (threshold !== undefined) {
            ...
            // No threshold provided, include all samples
            scoredSamples.push({
                ...sample,
                [scoreKey]: score,
            });
...
function rankBySimilarity(queryEmbedding, samples, options = {}) {
    ...
    // Select compute function based on method
